...
public class ProductServiceImpl implements ProductService {
     ...
     */
    @Override
    public SingleObject<Product> find(ProductSearchCondition condition, int productId) {
        String fashionGoApi = FashionGoApiConfig.fashionGoApi.concat("/v1.0/products/" + productId);
        ...
        try {
            //응답이 200이 아니거나 FashiongoAPI에 연결이 되지 않았을 경우 null을 반환함.
            FashionGoApiResponse<SingleObject<Product>> fashionGoApiResponse = client.get(fashionGoApi, header,
                    new ParameterizedTypeReference<FashionGoApiResponse<SingleObject<Product>>>() {});
