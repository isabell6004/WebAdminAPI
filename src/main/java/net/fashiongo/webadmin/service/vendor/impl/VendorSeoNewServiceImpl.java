	...
	public void modifyVendorSeo(Long vendorId, SetVendorSeoParameter request) {
        ...
        try {
            ...
            if (!fashionGoApiResponse.getHeader().isSuccessful()) {
                ...
                throw new VendorSeoOperationException("fail to update vendor seo. vendorId: " + vendorId);
            }
        } catch (IOException e) {
            log.error("fail to update vendor seo.", e);
            throw new VendorSeoOperationException("fail to update vendor seo. " + e.getMessage());
        }   	
    }
